...
fn solve(n: usize, _m: usize, v: Vec<usize>) -> bool {
    let mut k = vec![0usize; n + 1];
    for x in v {
        k[x] += 1;
    ...
    for i in 0..=n {
        if ((i == 0 || i == n) && k[i] == 1) || k[i] == 2 {
            let mut j = i;
            while k[j] > 0 && j < n {
                if k[j + 1] > 1 { return false; }
                j += 1;
            }
            let mut j = i;
            while k[j] > 0 && j > 0 {
...
fn read_eval_print<R: Read, W: Write>(io: &mut IO<R, W>) -> IOResult<()> {
    let n: usize = io.sp()?;
    let m: usize = io.ln()?;
    let v: Vec<usize> = split(&io.eof()?)?;
    let ans = solve(n, m, v);
    writeln!(io, "{}", if ans { "YES" } else { "NO" })?;
    Ok(())
}
