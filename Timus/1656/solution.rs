...
fn shift(r: usize, c: usize, x: usize) -> usize {
    let mut ans = 0usize;
    ans += if r > x { r - x } else { x - r };
    ans += if c > x { c - x } else { x - c };
    ans
}

fn solve(n: usize, mut v: Vec<u32>) -> Vec<Vec<u32>> {
    v.sort();
