...
fn solve(mut v: Vec<Student>) -> u32 {
    v.sort_by(|s1, s2| s1.t1.cmp(&s2.t1));
    let mut finish = 0u32;
    let mut ans = 0u32;
    for s in &v {
        finish = max(finish, s.t1) + s.t2;
        if finish > s.t3 { ans = max(ans, finish - s.t3); }
    }
    ans
}
...
fn solve_with_io<R: Read, W: Write>(io: &mut IO<R, W>) {
    ...
    for _ in 0..n {
        let (t1, t2, t3) = (io.sp::<u32>(), io.sp::<u32>(), io.ln::<u32>());
        v.push(Student { t1, t2, t3 });
    }
    let ans = solve(v);
