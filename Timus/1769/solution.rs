...
fn get_num(v: &[char]) -> usize {
    let mut ans = 0usize;
    for &c in v {
        ans = ans * 10 + c.to_digit(10).unwrap() as usize;
    }
    ans
}

fn solve(v: Vec<char>) -> usize {
    let n = v.len();
    let mut h = vec![false; N];
    for l in 1..=K {
        if l > n { break; }
        for i in 0..=(n - l) {
            let x = get_num(&v[i..(i + l)]);
            h[x] = true;
        }
    }
    for x in 1..N {
    ...
    panic!("The solution must have already been found!");
}
