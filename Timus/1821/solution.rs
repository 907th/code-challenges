...
fn solve(n: usize, mut v: Vec<Sportsmen>) -> Vec<String> {
    v.sort_by(|a, b| a.finished_at.cmp(&b.finished_at));
    let mut min_time = v[0].total_time;
    let mut ans: Vec<String> = Vec::new();
    ans.push(v[0].name.clone());
    for i in 1..n {
        if v[i].total_time < min_time {
            ans.push(v[i].name.clone());
            min_time = v[i].total_time;
        }
    }
    ans.sort();
    ans
...
fn solve_with_io<R: Read, W: Write>(io: &mut IO<R, W>) {
    let n: usize = io.ln();
    let mut v: Vec<Sportsmen> = Vec::new();
    for i in 0..n {
        let name: String = io.sp();
        let total_time = parse_time(io.ln());
        let finished_at = (i as i32) * 300 + total_time;
        v.push(Sportsmen { name, finished_at, total_time });
    }
    let ans = solve(n, v);
    writeln!(io.w, "{}", ans.len()).unwrap();
    for name in ans {
        writeln!(io.w, "{}", name).unwrap();
    }
}
